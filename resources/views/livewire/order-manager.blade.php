<div class="h-full flex flex-col">
    @if($order)
        ...
        <div class="flex-1 overflow-y-auto p-4">
            <div class="mb-4">
                <h3 class="font-semibold">Order #{{ $order->order_no }}</h3>
                <p class="text-sm text-gray-500">{{ $order->created_at->format('M d, Y h:i A') }}</p>
            </div>

            <div class="space-y-2 mb-4">
                @foreach($order->items->where('is_voided', false) as $item)
                    <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                        <div class="flex-1">
                            ...
                            @if($item->special_instructions)
                                <div class="text-xs text-gray-400">{{ $item->special_instructions }}</div>
                            @endif
                        </div>
